...
def merge_image(sub_image_dir, sub_size, overlay_size, target_path, target_shape):
    ...
    sub_image_files = list(filter(lambda filename: os.path.splitext(filename)[1] == '.tif', os.listdir(sub_image_dir)))
...
def main():
    ...
    ####################################################################
    # sub_size = (400, 400)
    # overlay_size = (160, 160)
    # # split_image(image_path, sub_size, overlay_size)
    # image_dir = r'D:\小论文2\区域分割\普适性检测3\ClipData'
    ...
    sub_size = (512, 512)
    overlay_size = (8, 8)
    target_shape = (10240,12800,3)
    sub_image_dir = r'F:\Download\bigmap\nantongshuangdianzhen\pl\NTcrop\split'
    target_path = r"F:\Download\bigmap\nantongshuangdianzhen\pl\NTcrop\mp.tif"
