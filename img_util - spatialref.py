...
# 多张图片赋参考系
def copy_spatialref_g(img_path4, img_path2):
    image_ds4 = gdal.Open(img_path4, gdal.GA_ReadOnly)
    if not image_ds4:
        print("Fail to open image {}".format(img_path4))
        return False
    ...
    image_dir2 = sorted(os.listdir(img_path2))
    for item in range(len(image_dir2)):
        image_ds2 = gdal.Open(img_path2 + '/' + image_dir2[item], gdal.GA_Update)
        if not image_ds2:
            print("Fail to open image {}".format(img_path2))
            return False
...
def main():
    ...
    # image_dir = r'F:\Download\bigmap\张家港'
    ...
    ####################################################################
    ### copy spatial_ref src_image -> merge_images
    # ####################################################################
    img_path2 = r'E:\plough_data\chongqing\img'
    img_path4 = r'F:\Download\bigmap\重庆\重庆_Level_18_crop.tif'
    copy_spatialref_g(img_path4, img_path2)
